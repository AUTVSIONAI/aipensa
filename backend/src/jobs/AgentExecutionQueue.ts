...
export default {
  ...
  async handle({ data }: { data: { taskId: string } }) {
    ...
    if (task.status !== "pending") {
      console.log(
        `[AgentWorker] Task ${taskId} status is ${task.status}, skipping.`
      );
      return;
    }
    ...
    try {
      ...
      const { accessToken, adAccountId } = await SocialMediaService.getFbConfig(
        companyId
      );
      ...
      switch (task.type) {
        case "instagram_post": {
          ...
          if (!instagramId) {
            throw new Error(
              "Instagram ID not provided and could not be resolved."
            );
          }
        ...
        case "ads_campaign": {
          ...
          result = await SocialMediaService.createFullAdCampaign(
            companyId,
            task.payload
          );
          break;
        ...
        case "whatsapp_status": {
          const { media_type, caption, image_url, video_url, whatsappId } =
            task.payload;
