...
module.exports = {
  up: async (queryInterface, Sequelize) => {
    ...
    if (
      !indexNamesMessage.some(index => index.name === "idx_message_ticket_id")
    ) {
    ...
    if (
      !indexNamesMessage.some(index => index.name === "idx_message_company_id")
    ) {
    ...
    if (
      !indexNamesTicket.some(index => index.name === "idx_ticket_contact_id")
    ) {
      await queryInterface.addIndex("Tickets", ["contactId"], {
        name: "idx_ticket_contact_id"
      });
    }
    if (
      !indexNamesTicket.some(index => index.name === "idx_ticket_whatsapp_id")
    ) {
      await queryInterface.addIndex("Tickets", ["whatsappId"], {
        name: "idx_ticket_whatsapp_id"
      });
    }
    if (!indexNamesTicket.some(index => index.name === "idx_ticket_queue_id")) {
      await queryInterface.addIndex("Tickets", ["queueId"], {
        name: "idx_ticket_queue_id"
      });
    }
    if (!indexNamesTicket.some(index => index.name === "idx_ticket_id")) {
      await queryInterface.addIndex("Tickets", ["id"], {
        name: "idx_ticket_id"
      });
    }
    if (
      !indexNamesTicket.some(index => index.name === "idx_ticket_company_id")
    ) {
    ...
    if (
      !indexNamesContactTag.some(index => index.name === "idx_contactTag_tagId")
    ) {
      await queryInterface.addIndex("ContactTags", ["tagId"], {
        name: "idx_contactTag_tagId"
      });
    }

    // Criação de índice para a tabela Queues
    if (!indexNamesQueue.some(index => index.name === "idx_queues_id")) {
      await queryInterface.addIndex("Queues", ["id"], {
        name: "idx_queues_id"
      });
    }
    ...
    if (
      !indexNamesMessage.some(index => index.name === "idx_message_quoted_id")
    ) {
      await queryInterface.addIndex("Messages", ["quotedMsgId"], {
        name: "idx_message_quoted_id"
      });
    }

    if (!indexNamesContact.some(index => index.name === "idx_contact_id")) {
      await queryInterface.addIndex("Contacts", ["id"], {
        name: "idx_contact_id"
      });
    }
    if (
      !indexNamesContact.some(index => index.name === "idx_contact_company_id")
    ) {
    ...
    if (
      !indexNamesContact.some(index => index.name === "idx_contact_whatsapp_id")
    ) {
      await queryInterface.addIndex("Contacts", ["whatsappId"], {
        name: "idx_contact_whatsapp_id"
      });
    }
    if (
      !indexNamesContactTag.some(
        index => index.name === "idx_contactTag_contact_id"
      )
    ) {
      await queryInterface.addIndex("ContactTags", ["contactId"], {
        name: "idx_contactTag_contact_id"
      });
    }
    if (
      !indexNamesContactTag.some(
        index => index.name === "idx_contactTag_tag_id"
      )
    ) {
      await queryInterface.addIndex("ContactTags", ["tagId"], {
        name: "idx_contactTag_tag_id"
      });
    }
