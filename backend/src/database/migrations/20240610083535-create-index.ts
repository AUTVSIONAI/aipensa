...
module.exports = {
  up: async (queryInterface, Sequelize) => {
    ...
    const indexNamesTicketTraking = await queryInterface.showIndex(
      "TicketTraking"
    );
    ...
    const indexNamesContactCustomFields = await queryInterface.showIndex(
      "ContactCustomFields"
    );
    ...
    if (
      !indexNamesTicketTraking.some(
        index => index.name === "idx_TicketTraking_ticket_id"
      )
    ) {
      await queryInterface.addIndex("TicketTraking", ["ticketId"], {
        name: "idx_TicketTraking_ticket_id"
      });
    }
    if (
      !indexNamesTicketTraking.some(
        index => index.name === "idx_TicketTraking_company_id"
      )
    ) {
      await queryInterface.addIndex("TicketTraking", ["companyId"], {
        name: "idx_TicketTraking_company_id"
      });
    }

    if (
      !indexNamesUserRatings.some(
        index => index.name === "idx_userratings_company_id"
      )
    ) {
      await queryInterface.addIndex("UserRatings", ["companyId"], {
        name: "idx_userratings_company_id"
      });
    }

    if (
      !indexNamesUserRatings.some(
        index => index.name === "idx_userratings_ticket_id"
      )
    ) {
      await queryInterface.addIndex("UserRatings", ["ticketId"], {
        name: "idx_userratings_ticket_id"
      });
    }

    if (
      !indexNamesMessages.some(
        index => index.name === "idx_Messages_contact_id"
      )
    ) {
    ...
    if (
      !indexNamesLogTickets.some(
        index => index.name === "idx_LogTickets_ticket_id"
      )
    ) {
    ...
    if (
      !indexNamesContactCustomFields.some(
        index => index.name === "idx_ContactCustomFields_contact_id"
      )
    ) {
    ...
    if (
      !indexNamesTicketTags.some(
        index => index.name === "idx_TicketTags_ticket_id"
      )
    ) {
      await queryInterface.addIndex("TicketTags", ["ticketId"], {
        name: "idx_TicketTags_ticket_id"
      });
    }

    if (
      !indexNamesTicketTags.some(
        index => index.name === "idx_TicketTags_tag_id"
      )
    ) {
      await queryInterface.addIndex("TicketTags", ["tagId"], {
        name: "idx_TicketTags_tag_id"
      });
    }
  },
  ...
  down: async (queryInterface, Sequelize) => {
    ...
    await queryInterface.removeIndex(
      "TicketTraking",
      "idx_TicketTraking_ticket_id"
    );
    await queryInterface.removeIndex(
      "TicketTraking",
      "idx_TicketTraking_company_id"
    );
    ...
    await queryInterface.removeIndex(
      "ContactCustomFields",
      "idx_ContactCustomFields_contact_id"
    );
    await queryInterface.removeIndex(
      "UserRatings",
      "idx_userratings_company_id"
    );
    await queryInterface.removeIndex(
      "UserRatings",
      "idx_userratings_ticket_id"
    );
