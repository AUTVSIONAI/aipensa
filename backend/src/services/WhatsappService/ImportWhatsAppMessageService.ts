...
const ImportWhatsAppMessageService = async (whatsappId: number | string) => {
  ...
  try {
    ...
    Criação do arquivo de logs: ${format(new Date(), "dd/MM/yyyy HH:mm:ss")}
    Selecionado Data de inicio de importação: ${format(
      new Date(dateOldLimit),
      "dd/MM/yyyy HH:mm:ss"
    )} 
    Selecionado Data final da importação: ${format(
      new Date(dateRecentLimit),
      "dd/MM/yyyy HH:mm:ss"
    )} 
    `
    });
...
Mensagem ${i + 1} de ${qtd}
          ...
          io.of(whatsApp.companyId.toString()).emit(
            `importMessages-${whatsApp.companyId}`,
            {
              ...
              status: {
                this: i + 1,
                all: qtd,
                date: format(new Date(timestampMsg), "dd/MM/yy HH:mm:ss")
              }
            }
          );
        }
        ...
        if (i + 1 === qtd) {
          ...
          await whatsApp.update({
            statusImportMessages: whatsApp.closedTicketsPostImported
              ? null
