...
export const ActionsWebhookFacebookService = async (
  ...
  for (var i = 0; i < lengthLoop; i++) {
    ...
    console.log(`[${i}] Processando node:`, {
      id: nodeSelected.id,
    ...
    if (nodeSelected.type === "singleBlock") {
      for (var iLoc = 0; iLoc < nodeSelected.data.seq.length; iLoc++) {
        ...
        if (elementNowSelected.includes("img")) {
          ...
          //Obtendo o nome do arquivo sem a extensÃ£o
    ...
    if (nodeSelected.type === "interval") {
      ...
      console.log(`Iniciando timer de ${timerSeconds} segundos...`);

      await new Promise(resolve => {
        setTimeout(() => {
          console.log(`Timer de ${timerSeconds} segundos finalizado.`);
          resolve(true);
        }, timerSeconds * 1000);
      });
...
const intervalWhats = (time: string) => {
  ...
  console.log(`IntervalWhats: aguardando ${time} segundos (${seconds}ms)`);

  return new Promise(resolve => {
    setTimeout(() => {
      console.log(`IntervalWhats: ${time} segundos concluído`);
      resolve(seconds);
    }, seconds);
  });
};
