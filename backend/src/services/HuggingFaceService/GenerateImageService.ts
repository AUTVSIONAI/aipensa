...
const GenerateImageService = async ({
  ...
  if (!resolvedApiKey) {
    throw new Error(
      "HUGGINGFACE_API_KEY is not defined in environment variables or settings."
    );
  }
  ...
  for (const currentModel of uniqueModels) {
    ...
    try {
      console.log(
        `[HuggingFaceService] Generating image with model ${currentModel}...`
      );
      ...
      console.log(
        `[HuggingFaceService] Image generated successfully with ${currentModel}: ${url}`
      );
      ...
      console.error(
        `[HuggingFaceService] Error generating image with ${currentModel}:`,
  ...
  throw new Error(
    `Failed to generate image after trying models: ${uniqueModels.join(
      ", "
