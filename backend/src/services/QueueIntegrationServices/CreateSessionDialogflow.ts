...
const createDialogflowSession = async (
  id: number,
  projectName: string,
  jsonContent: string
): Promise<SessionsClient | undefined> => {
  if (sessions.has(id)) {
  ...
  logger.info(
    `Openig new dialogflow session #${projectName} in '${keyFilename}'`
  );
...
const createDialogflowSessionWithModel = async (
  ...
  return createDialogflowSession(
    model.id,
    model.projectName,
    model.jsonContent
  );
